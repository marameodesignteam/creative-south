...
function seopress_pro_admin_std_metaboxe_display() {
    ...
    function seopress_pro_cpt($post) {
        ...
        wp_enqueue_script('seopress-pro-media-uploader-js', plugins_url('assets/js/seopress-pro-media-uploader' . $prefix . '.js', dirname(dirname(__FILE__))), ['jquery'], SEOPRESS_PRO_VERSION, false);
        wp_enqueue_script('seopress-pro-rich-snippets-js', plugins_url('assets/js/seopress-pro-rich-snippets' . $prefix . '.js', dirname(dirname(__FILE__))), ['jquery', 'jquery-ui-tabs'], SEOPRESS_PRO_VERSION, false);
        wp_enqueue_media();
		...
		<div id="seopress-schemas-tabs">
			<ul class="wrap-schemas-list">
				<li><a href="#seopress-schemas-tabs-1"><?php _e('Manual', 'wp-seopress-pro'); ?></a></li>
				<li><a id="sp-automatic-tab" href="#seopress-schemas-tabs-2"><?php _e('Automatic', 'wp-seopress-pro'); ?><span></span></a></li>
			</ul>

			<template id="js-select-template-schema">
				<div class="box-schema-item" data-key="[X]">
					<div class="wrap-rich-snippets-type" style="display:flex; align-items:flex-end; border-bottom:1px solid #d4d4d4; margin-bottom:15px;">
						...
						<div>
							...
							<select id="seopress_pro_rich_snippets_type" class="js-select_seopress_pro_rich_snippets_type" name="seopress_pro_rich_snippets_data[X][seopress_pro_rich_snippets_type]">
								<option value="none"><?php _e('None', 'wp-seopress-pro'); ?></option>
								<?php foreach ($options_schemas_available as $item) { ?>
									<option value="<?php echo $item['value']; ?>"><?php echo $item['label']; ?></option>
								<?php } ?>
							</select>
						</div>
						<a href="#" class="js-delete-schema-manual button" style="margin-left:auto; height:100%; margin-bottom:10px;" data-key="[X]">
			...
			<?php foreach ($options_schemas_available as $item) { ?>
				<template id="schema-template-<?php echo $item['value']; ?>">
			...
			<template id="schema-template-none">
				<div class="wrap-rich-snippets-item">
					<ul class="advice seopress-list notice notice-info">
						<li class="warning"><span class="dashicons dashicons-warning"></span><?php _e('WARNING', 'wp-seopress-pro'); ?></li>
						<li><?php _e('Be sure to select the right structure data type for your content.', 'wp-seopress-pro'); ?></li>
						<li><?php _e('When you choose one, fill all fields with the right format.', 'wp-seopress-pro'); ?></li>
						<li><?php _e('Make sure you don\'t have already included structured data type with a theme or plugin (eg: the default WooCommerce Theme, Storefront, already implements this for single page products).', 'wp-seopress-pro'); ?></li>
						<li><?php _e('You can test your page with Google Data Structure Test tool.', 'wp-seopress-pro'); ?> <a href="https://search.google.com/test/rich-results" target="_blank"><?php _e('Make a test', 'wp-seopress-pro'); ?></a></li>
						<?php
						...
						<li>
							<?php
                                ...
                                echo sprintf(__('If you need help with schema, <a href="%s">read our full tutorial</a>.', 'wp-seopress-pro'), $seopress_docs_link['support']['schema']); ?>
						</li>
					</ul>
				</div>
			</template>
				...
				<div class="box-left">
					<div class="schemas-bar-new">
						<p style="padding-left:12px;">
							<a href="#" id="js-add-schema-manual" class="button button-primary">
								<?php _e('Add a schema', 'wp-seopress-pro'); ?>
							</a>
						</p>
						<p style="margin-left:auto; margin-right:12px;">
							<a href="#" class="js-expand-all"><?php _e('Expand', 'wp-seopress-pro'); ?></a>&nbsp;/&nbsp;<a href="#" class="js-close-all"><?php _e('Close', 'wp-seopress-pro'); ?></a>
						</p>
					</div>
					<div id="js-box-list-schemas">
						<?php if ($is_multidimensional) { ?>
							...
							<div class="box-schema-item" data-key="<?php echo $key; ?>">
								<div class="wrap-rich-snippets-type" style="display:flex; align-items:flex-end; border-bottom:1px solid #d4d4d4; margin-bottom:15px;">
									<button type="button" class="js-handle-snippet-type" aria-expanded="true">
										<span class="toggle-indicator" aria-hidden="true"></span>
									</button>
									<div>
										<label for="seopress_pro_rich_snippets_type_meta"><?php _e('Select your data type', 'wp-seopress-pro'); ?></label>
										<select
											id="seopress_pro_rich_snippets_type"
										...
										>
											<option <?php echo selected('none', $seopress_pro_rich_snippets_type); ?> value="none"><?php _e('None', 'wp-seopress-pro'); ?></option>
											<?php foreach ($options_schemas_available as $item) { ?>
												<option <?php echo selected($item['value'], $seopress_pro_rich_snippets_type); ?> value="<?php echo $item['value']; ?>"><?php echo $item['label']; ?></option>
											<?php } ?>
										</select>
									</div>

									<a href="#" class="js-delete-schema-manual button" style="margin-left:auto; height:100%; margin-bottom:10px;" data-key="<?php echo $key; ?>">
										<?php _e('Delete schema', 'wp-seopress-pro'); ?>
									</a>
								</div>
						...
						<?php } else {
							...
							<div class="box-schema-item" data-key="0">
								<div class="wrap-rich-snippets-type" style="display:flex; align-items:flex-end;border-bottom:1px solid #d4d4d4">
									<div>
										<label for="seopress_pro_rich_snippets_type_meta"><?php _e('Select your data type', 'wp-seopress-pro'); ?></label>
										<select
											id="seopress_pro_rich_snippets_type"
					...
					<p style="padding-left:12px;">
						<a href="https://search.google.com/test/rich-results?url=<?php echo get_permalink(); ?>" target="_blank" class="button">
							<?php _e('Validate my schema', 'wp-seopress-pro'); ?>
						</a>
					</p>
				</div>
			</div>
			<div id="seopress-schemas-tabs-2" style="padding-left:12px; padding-right:12px;">
