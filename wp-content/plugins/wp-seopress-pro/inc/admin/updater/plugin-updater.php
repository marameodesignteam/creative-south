...
class SEOPRESS_Updater {
    ...
    public function show_update_notification($file, $plugin) {
        ...
        if ( ! empty($update_cache->response[$this->name]) && version_compare($this->version, $version_info->new_version, '<')) {
            ...
            echo '<div class="update-message seopress-notice inline is-warning">';
    ...
    private function api_request($_action, $_data) {
        ...
        if ( ! is_array($edd_plugin_url_available) || ! isset($edd_plugin_url_available[$store_hash])) {
            ...
            if (empty($host)) {
                ...
                $response                              = wp_remote_get($test_url, ['user-agent' =>'WordPress/' . get_bloginfo('version'), 'timeout' => $this->health_check_timeout, 'sslverify' => $verify_ssl]);
        ...
        $request    = wp_remote_post($this->api_url, ['user-agent' =>'WordPress/' . get_bloginfo('version'), 'timeout' => 15, 'sslverify' => $verify_ssl, 'body' => $api_params]);
