...
jQuery(document).ready(function($){
	...
	$("#js-add-schema-manual").on("click", function(e){
		...
		str = str.replace("[X]", key).replace("seopress_pro_rich_snippets_data[X]", "seopress_pro_rich_snippets_data[" + key + "]").replace("[X]", key)
	...
	$(document).on("click", ".js-delete-schema-manual", function(e){
		...
		if($(".box-schema-item").length === 0){
			...
			str = str.replace("[X]", number).replace("seopress_pro_rich_snippets_data[X]", "seopress_pro_rich_snippets_data[" + number + "]").replace("[X]", number)
	 ...
	 */
	$(document).on("change", ".js-select_seopress_pro_rich_snippets_type", function(e){
		e.preventDefault()

		const _self = $(this)
		const value = $(this).val()

		const template = document.querySelector("#schema-template-" + value);
		if(template.length === 0){
			return
		}
		...
		let number = null
		const alreadyExistItem = getContainerItem().find(".wrap-rich-snippets-item:eq(0)").hasClass("wrap-rich-snippets-" + snippet_selected)

		if(alreadyExistItem){
			getContainerItem().find(".wrap-rich-snippets-item:eq(0)").show()
		}
		else{
			...
			str = str.split(find).join('seopress_pro_rich_snippets_data[' + number + ']');
	...
	function bindSnippetArticleCounter(number){
		const selector = ".box-schema-item[data-key='" + number + "'] .seopress_rich_snippets_articles_counters"

		$(selector).after("<div class=\"seopress_rich_snippets_articles_counters_val\">/ 110</div>");

		if ($(selector).length != 0) {

			$(selector).text($(".box-schema-item[data-key='" + number + "'] .seopress_pro_rich_snippets_article_title_meta").val().length);
			if($(".box-schema-item[data-key='" + number + "'] .seopress_pro_rich_snippets_article_title_meta").val().length > 110){
				$(selector).css('color', 'red');
			}
			$(".box-schema-item[data-key='" + number + "'] .seopress_pro_rich_snippets_article_title_meta").keyup(function(event) {
	...
	function bindSnippetCourseDescription(number){
		const selector = ".box-schema-item[data-key='" + number + "'] .seopress_rich_snippets_courses_counters"

		$(selector).after("<div id=\"seopress_rich_snippets_courses_counters_val\">/ 60</div>");

		if ($(selector).length != 0) {
			$(selector).text($(".box-schema-item[data-key='" + number + "'] .seopress_pro_rich_snippets_courses_desc").val().length);
			if($(".box-schema-item[data-key='" + number + "'] .seopress_pro_rich_snippets_courses_desc").val().length > 60){
				$(selector).css('color', 'red');
			}
			$(".box-schema-item[data-key='" + number + "'] .seopress_pro_rich_snippets_courses_desc").keyup(function(event) {
				$(selector).css('color', 'inherit');
	...
	function bindUploadMedia(number){
		const selector = ".box-schema-item[data-key='" + number + "'] .seopress_media_upload"
		if($(selector).length === 0){
			return
		}
    ...
    //Employment Type
    $(document).on("click",'#seopress-tag-employment-1, #seopress-tag-employment-2, #seopress-tag-employment-3, #seopress-tag-employment-4, #seopress-tag-employment-5, #seopress-tag-employment-6, #seopress-tag-employment-7, #seopress-tag-employment-8', function() {
	...
	function seopress_call_faq_accordion(number) {
		...
		if($( ".box-schema-item[data-key='" + number + "'] #wrap-faq .faq" ).length === 0){
			return;
		}
		$( ".box-schema-item[data-key='" + number + "'] #wrap-faq .faq" ).accordion({
			collapsible: true,
			active: false,
			heightStyle:"panel",
			header: 'h3'
	...
	function bindAddFaq(number){
		...
		$(".box-schema-item[data-key='" + number + "'] #add-faq").click(function(e) {
			...
			var section = template.clone().find(':input').each(function(){
				...
				var newId = this.id.replace(/(\[seopress_pro_rich_snippets_faq\])\[.*?\]/, '$1['+sectionsCount+']');

				//Update input name
				$(this).attr('name', input_name.replace(/(\[seopress_pro_rich_snippets_faq\])\[.*?\]/, '$1['+sectionsCount+']'));

				//update for label
				$(this).prev().attr('for', input_id.replace(/(\[seopress_pro_rich_snippets_faq\])\[.*?\]/, '$1['+sectionsCount+']'));
				$(this).prev().attr('id', input_name.replace(/(\[seopress_pro_rich_snippets_faq\])\[.*?\]/, '$1['+sectionsCount+']'));
			...
			//inject new section
			section.appendTo(".box-schema-item[data-key='" + number + "'] #wrap-faq");
    ...
    $(document).on('click', '.wrap-rich-snippets-rules #seopress_pro_rich_snippets_rules_add', function(e) {
        // Clone a row.
	...
	if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'none') {
        $(sc_ad).show();
    } else if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'articles') {
        $(sc_ad).hide();
        $(sc_a).show();
    } else if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'localbusiness') {
        $(sc_ad).hide();
        $(sc_b).show();
    } else if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'faq') {
        $(sc_ad).hide();
        $(sc_f).show();
    } else if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'courses') {
        $(sc_ad).hide();
        $(sc_c).show();
    } else if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'recipes') {
        $(sc_ad).hide();
        $(sc_r).show();
    } else if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'jobs') {
        $(sc_ad).hide();
        $(sc_j).show();
    } else if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'videos') {
        $(sc_ad).hide();
        $(sc_v).show();
    } else if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'events') {
        $(sc_ad).hide();
        $(sc_e).show();
    } else if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'products') {
        $(sc_ad).hide();
        $(sc_p).show();
    } else if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'services') {
        $(sc_ad).hide();
        $(sc_s).show();
    } else if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'softwareapp') {
        $(sc_ad).hide();
        $(sc_app).show();
    } else if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'review') {
        $(sc_ad).hide();
        $(sc_re).show();
    } else if ($("#seopress-your-schema #seopress_pro_rich_snippets_type option:selected").val() == 'custom') {
        $(sc_ad).hide();
        $(sc_cu).show();
    }

	$('#seopress-your-schema #seopress_pro_rich_snippets_type').change(function() {
		var seopress_rs_type = $('#seopress-your-schema #seopress_pro_rich_snippets_type').val();
